    ...
    sNode.labels.forEach(value => {
      if (this.style.labels) {
        ...
        if (style) {
          ...
          if (style.icon) sNode.icon = style.icon;

          if (style.type) sNode.type = style.type;

          sNode.type = style.type || "circle";
          if (style.label && sNode.properties[style.label])
            sNode.label = sNode.properties[style.label];
        }
      }
    });
  ...
  _neo4jToSigmaEdge(edge) {
    ...
    if (this.style.edges) {
      ...
      if (style) {
        ...
        sEdge.type = style.type || "paracurve";
        if (style.label && sEdge.properties[style.label])
          sEdge.label = sEdge.properties[style.label];
      }
    }
