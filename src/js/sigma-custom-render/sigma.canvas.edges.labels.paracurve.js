(function(undefined) {
   ...
   */
  sigma.canvas.edges.labels.paracurve = function(
    edge,
    source,
    target,
    context,
    ...
    if (
      !hidden &&
      (typeof edge.label === "string" || Array.isArray(edge.label)) &&
      edgeSize >= settings("edgeLabelThreshold")
    ) {
      var text = edge.label;
      if (Array.isArray(edge.label)) {
        text = edge.label.join(", ");
      }
      var controlPoint, point, angle;
      if (source.id === target.id) {
        controlPoint = sigma.canvas.utils.getSelfLoopControlPoints(
          sourceX,
          sourceY,
          source[prefix + "size"],
          edgeSize,
        ...
        point = sigma.utils.getPointOnBezierCurve(
          0.5,
          sourceX,
          sourceY,
          targetX,
        ...
        controlPoint = sigma.canvas.utils.getQuadraticControlPoint(
          sourceX,
          sourceY,
          targetX,
          targetY,
        ...
        point = sigma.utils.getPointOnQuadraticCurve(
          0.5,
          sourceX,
          sourceY,
          targetX,
      ...
      if (settings("edgeLabelSize") !== "fixed") {
        fontSize =
          fontSize *
          edgeSize *
          Math.pow(edgeSize, -1 / settings("edgeLabelSizePowRatio"));
      }
      context.font = [
